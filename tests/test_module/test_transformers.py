...
def load_roberta_model(**kwargs):
    return tr.load_model('roberta-pred-mean-pool',
...
def test_roberta_mean_pool_load_new():
    try:
        ...
        raise Exception('Failed to load new RobertaForSequenceClassificationMeanPool model')


def test_roberta_mean_pool_load_pretrained():
    try:
        load_roberta_model(pretrained_model=config.models / 'transformer' / 'language-model')
    except:
        raise Exception('Failed to load pretrained RobertaForSequenceClassificationMeanPool model')
...
def test_delay_scheduler():
    tr._get_scheduler('delay', Adam(), 10000,
...
def test_convert_str_to_list():
    ...
    assert all([all([type(d) == float for d in li]) for li in lists]), 'Elements are not float'
