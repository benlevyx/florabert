...
def main():
    args = utils.get_args(data_dir=DATA_DIR,
    ...
    config_obj, tokenizer, model = tr.load_model(args.model_name,
                                                 TOKENIZER_DIR,
    ...
    datasets = dataio.load_datasets(tokenizer,
                                    args.train_data,
    ...
    data_collator = dataio.load_data_collator('language-model',
                                              tokenizer=tokenizer,
                                              mlm_prob=config.settings['training']['pretrain'].pop('mlm_prob'))

    training_settings = config.settings['training']['pretrain']
    trainer = training.make_trainer(model, data_collator, dataset_train,
                                    dataset_test, args.output_dir,
                                    **training_settings)
