...
if __name__ == '__main__':
    ...
    if db_name not in ['Ensembl', 'Refseq', 'Maize', 'Maize_addition', 'Maize_nam']:
        raise ValueError("Arg db_name must be one of ['Ensembl', 'Refseq', 'Maize', 'Maize_addition', 'Maize_nam'].")
    print("Name of database to be processed:", db_name)

    # Read in the db_link.csv
    df = pd.read_csv(os.path.join(config.data_raw, "gz_link", f"{db_name.lower()}_link.csv"))
    ...
    for idx, row in df.iterrows():
        ...
        try:
            gene_db_io.generate_sequence_for_species(dna_name, annot_name, dna_url, annot_url,
                                                     dna_path, annot_path, processed_db_path,
                                                     db_name, species_name, regulatory_len=1000)

            print(f"\nFinished processing index {idx}:", dna_name)
            
            # Test load process dna sequences
            processed_fa = gene_db_io.load_processed_fa(processed_db_path,
                                                        dna_name,
                                                        db_name,
                                                        species_name)
